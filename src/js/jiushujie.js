...
window.onload = () => {
  ...
  body.prepend(status_flag)

  const details = document.querySelectorAll('#main_content .book_item .detail')

  append_links(details)
}

// 从页面DOM中解析条形码，据以生成并插入在当当网搜索相应商品的链接。
const append_links = doms => {
  doms.forEach(dom => {
    let barcode = getBarcode(dom) // 解析当前DOM所含二维码

    // append_css('https://cdn.liuyajie.com/bootstrap/4.4.1/bootstrap.min.css') // 添加CSS，已测试通过，为减少CDN成本改为直接向链接DOM添加样式的方式↓
    if (barcode && barcode.length === 13) append_link(barcode, dom) // 添加链接
  })
}
